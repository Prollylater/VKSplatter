#include "ResourceSystem.h"
    ...
    AssetID<Mesh> AssetSystem::loadMeshWithMaterials(const std::string &filename)
    {
        ...
        for (const auto &mtl : materials)
        {
            ...
            {
                auto texture = std::make_unique<Texture>(mtl.diffuse_texname);
                AssetID textureID = mRegistry.add<Texture>(mtl.diffuse_texname, std::move(texture));
                material->albedoMap = textureID;
            }

            if (!mtl.bump_texname.empty())
            {
                auto texture = std::make_unique<Texture>(mtl.bump_texname);
                AssetID textureID = mRegistry.add<Texture>(mtl.bump_texname, std::move(texture));
                material->normalMap = textureID;
            }
            if (!mtl.metallic_texname.empty())
            {
                auto texture = std::make_unique<Texture>(mtl.metallic_texname);
                AssetID textureID = mRegistry.add<Texture>(mtl.metallic_texname, std::move(texture));
    ...
    AssetID<Mesh> AssetSystem::loadMesh(const std::string &path)
    {
        ...
        // Optionally set mesh->name = path; if Mesh has name member
        return mRegistry.add<Mesh>(path, std::move(meshPtr));
    }

    AssetID<Texture> AssetSystem::loadTexture(const std::string &path)
    {
        ...
        return mRegistry.add<Texture>(path, std::move(texPtr));
    }
